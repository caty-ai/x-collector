...
export default function QiitaSourcesPanel() {
  const [sources, setSources] = useState<QiitaSource[]>([]);
  ...
  const fetchSources = useCallback(async () => {
    const res = await fetch("/api/qiita-sources");
  ...
  async function handleFetchOne(id: number, deepItems?: number) {
    ...
    const res = await fetch("/api/qiita-fetch", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
<div className="mb-8 border-b border-hairline pb-4">
        <Headline level="sm" as="h2">Qiita Sources</Headline>
        <p className="mt-1 font-sans text-wired-meta uppercase tracking-widest text-ink-soft">
          Manage Qiita tag sources to collect articles from
        </p>
      </div>
      ...
      <form onSubmit={handleAdd} className="mb-8 space-y-4 border border-hairline bg-paper p-5">
        <h3><Eyebrow>Add New Qiita Source</Eyebrow></h3>
        ...
        <div>
          <label className="mb-1 block font-sans text-wired-meta font-bold uppercase tracking-widest text-ink-soft">Qiita Tag</label>
          <input
            type="text"
            placeholder='e.g. "ChatGPT" or "qiita.com/tags/ChatGPT"'
            value={tag}
          ...
          <p className="mt-1 font-sans text-wired-meta text-ink-soft">
            Qiita tag name to collect articles from. Rate limit: 60 req/hour without auth.
          </p>
        </div>
      ...
      <div className="space-y-3">
        {sources.map((s) => (
          ...
          >
            ...
            <p className="font-sans text-wired-meta tabular-nums text-ink-soft">
              Routine: {s.maxItems} items · Added {new Date(s.createdAt).toISOString().slice(0, 10)}
        ...
        {sources.length === 0 && (
          <div className="border border-hairline py-16 text-center font-sans text-sm text-ink-soft">
            No Qiita sources yet. Add one above.
          </div>
        )}
      </div>
    </div>
