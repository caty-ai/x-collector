...
export default async function QiitaItemsPage({
  searchParams,
}: {
  ...
  const allSources = await prisma.qiitaSource.findMany({
  ...
  const totalCount = await prisma.qiitaItem.count({ where });
  ...
  const totalItems = await prisma.qiitaItem.count();
  ...
  function buildUrl(params: Record<string, string>) {
    ...
    const qs = p.toString();
    return qs ? `/qiita-items?${qs}` : "/qiita-items";
  }
  ...
  return (
    <main className="max-w-4xl mx-auto px-5 py-8">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight">Qiita Items</h1>
        <p className="text-muted-foreground text-sm mt-1">
          {totalItems} items collected from {allSources.length} Qiita tags
        </p>
      </div>
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id} className="border border-hairline bg-paper p-5">
      ...
      {items.length === 0 && (
        <div className="text-center text-muted-foreground py-16">
          No Qiita items yet. Add sources in <a href="/qiita-sources" className="text-link underline">Qiita Sources</a>.
        </div>
      )}
