...
import { crosslinkPipelineItems } from "../lib/pipeline/crosslink";
...
async function main() {
  ...
  try {
    const result = await crosslinkPipelineItems(prisma, {
...
main().catch((error) => {
  console.error("Fatal crosslink error:", error);
  process.exit(1);
});
