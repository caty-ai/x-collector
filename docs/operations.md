...
- **AI 向け案内板と AI に聞くプロンプト** — `/llms.txt` は `NEWSPAPER_PUBLIC` の on/off にかかわらず誰にでも配信され、Markdown 版の紙面 URL（`/api/bff/newsletter-editions/latest?format=markdown&date=YYYY-MM-DD`）と HTML 紙面ページを案内するだけで、紙面本文は公開しない。読者向けの「この日の新聞をAIに聞く」プロンプトも Markdown 版を先に開き、読めない場合だけ HTML 紙面へ進むよう案内する。両 URL の紙面を外部 AI が読めるのは `NEWSPAPER_PUBLIC` が on のときだけで、off の場合、匿名の BFF 取得は常に 401、HTML 紙面ページは `/np-login` へ転送されるため外部 AI は読めず（404 は公開モードで未公開・存在しない日付のときだけ）、プロンプトのフォールバック文に従ってその旨を伝える。

- 公開モードの匿名 newsletter BFF は、検証済みの `date`、`format`、`includeContent`、`includeItems` だけを受け取り、upstream へ `status=published` と `projection=public` を常に付ける。`slug` その他の parameter は破棄する。upstream は published 以外を 404 にし、BFF の `x-edition-status` / `edition.status` check は第二防衛線として残す。BFF も同じ public projection を再適用するため field guard は Railway の deploy 順に依存しないが、推奨順は引き続き upstream を先に deploy する。
...
- `/api/newsletter-editions/month` は additive であり、web が upstream より先に deploy された場合、upstream が追いつくまで calendar はエラー banner を表示し、BFF logs には coded 404 が残る（per-day fallback は行わない）。
...
- allowlistからアドレスを削除した場合、削除後はmiddleware／session gateが各リクエストで再評価するため保護ページは403・session必須APIは401になる（実効的な即時遮断）。JWT 自体はブラウザからの `/api/auth/session` が成功するたびに再署名され、cookie が更新される（スライディング）。サーバ側の `getServerSession(authOptions)`（App Router / RSC 経路）も内部で再エンコードするが、更新後の cookie はクライアントへ書き戻さない。`updateAge`（既定24時間）は database session 方式にのみ適用され、本リポジトリの JWT 方式では参照されない。既定 `maxAge`（30日）の満了は当てにできず、トークンそのものを確実に失効させる手段は `AUTH_SECRET`/`NEXTAUTH_SECRET` のローテーションのみ。
- BFF 3 route は `getServerSession` の結果に対して `ADMIN_EMAIL_ALLOWLIST` を `resolveAllowlistedSession` で再判定する。これはsession callbackの空session依存に対するdefence in depthであり、その空bodyを `null` にするnext-auth v4の挙動を固定するため `next-auth` は `~4.24.13` にpinする。
- `/calendar` の `/np-login` 共有パスワード経路はNextAuthの外側なので機能自体に変更はない。allowlist外のGoogleアカウントはGoogle経由では `/calendar` に入れない（`NEWSPAPER_PUBLIC` が off のとき）が、有効な共有クッキーは従来どおり利用できる。
