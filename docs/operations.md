...
- 公開モードの匿名 newsletter BFF は、検証済みの `date`、`format`、`includeContent`、`includeItems` だけを受け取り、upstream へ `status=published` と `projection=public` を常に付ける。`slug` その他の parameter は破棄する。upstream は published 以外を 404 にし、BFF の `x-edition-status` / `edition.status` check は第二防衛線として残す。
