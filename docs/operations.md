...
- allowlistからアドレスを削除した場合、削除後はmiddleware／session gateが各リクエストで再評価するため保護ページは403・session必須APIは401になる（実効的な即時遮断）。JWT 自体は session 取得（`/api/auth/session`・`getServerSession`）が成功するたびに再署名・再発行される（スライディング）。`updateAge`（既定24時間）は database session 方式にのみ適用され、本リポジトリの JWT 方式では参照されない。既定 `maxAge`（30日）の満了は当てにできず、トークンそのものを確実に失効させる手段は `AUTH_SECRET`/`NEXTAUTH_SECRET` のローテーションのみ。
