...
| OS（開発・セルフホスト） | WSL2（Windows 上の Ubuntu） | ⚠️ Linux 経路でカバー。個別の実走はなし。クローンは Linux ファイルシステム側（例: `~/`）に置き、`/mnt/c` は避けてください — drvfs 越しでは `next dev` のファイル監視が不安定で、`npm install` も大幅に遅くなります |
