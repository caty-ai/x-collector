...
3. **Reconciliation audit** (§5.3 live form; in `record-only` the reduced form). Runs **after** step 2 so that invitations that just expired are already re-armed in the ledger and are not counted as "predicted present". A divergence here sets precondition (e) false for the rest of this sweep.
