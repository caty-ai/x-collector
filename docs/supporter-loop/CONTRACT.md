...
4. **Least privilege, two single-purpose credentials.** Exactly two long-lived credentials exist (§8), both scoped to the reward repo only and never used against the source repo: `SUPPORTER_LEDGER_TOKEN` (Contents only — ledger and `SUPPORTERS.md`) and `SUPPORTER_LOOP_TOKEN` (Administration — invitations and revokes). The Administration-capable token is loaded **only** by the `act` job, which does not run in `record-only` (§4.4). Supporter-facing comments use the caller's `GITHUB_TOKEN`. **One read-only exception (v1.11):** the weekly sweep's stargazer listing (§11 precondition (b), `GET /repos/{source_repo}/stargazers`) is read with `SUPPORTER_LEDGER_TOKEN`, because the platform refuses that endpoint to `GITHUB_TOKEN` (403 "Resource not accessible by integration", even on a public repo) — a fine-grained PAT reads a public repository implicitly (§8.4) and this grants nothing on the source repo; like the §8.4 scope probes the exception covers that `GET` only, never a write, and the §4.4 static check keeps rejecting any write from `decide` outside `ledger/`.
...
The central workflow MUST be split into two delivery jobs (`decide`, `act`) plus a failure-`alert` job — three in total — so that *record-only cannot send* by construction:

| Job | Runs when | `permissions` | May use |
|---|---|---|---|
| `decide` | always | `contents: none`, `actions: read`, `issues: read`, `pull-requests: read`, `discussions: read` | `GITHUB_TOKEN` (read) · `SUPPORTER_LEDGER_TOKEN` (Contents only) in the steps that read/append the ledger (§6) and, in a sweep, for the read-only stargazer listing (§0-4 v1.11 exception) · **never** `SUPPORTER_LOOP_TOKEN` (the secret is not referenced anywhere in this job) · **no** Telegram secrets (§9 is live-only) |
...
- `SUPPORTER_LEDGER_TOKEN` is present in `decide` only for the ledger operations (read, append) and the read-only stargazer listing of the sweep (§0-4 v1.11), each with step-level `env:`; it cannot invite (no Administration permission) even if misused.
...
- v1.11 (2026-09-07, alpha) — **post-freeze correction** (checkpoint ④ follow-up, caty-ai/.github#89 / caty-ai/x-collector#139): the first live weekly sweep (run 34100523613, schedule 2026-09-07 08:25Z) failed closed with `sweep_gate=false` and an empty stargazer list; a throwaway probe (run 34107453239) measured two independent causes. (1) `GET /repos/{source_repo}/stargazers` returns 403 "Resource not accessible by integration" for `GITHUB_TOKEN` even on a public repo, so §0-4 gains a one-sentence **read-only exception**: the sweep reads the star list with `SUPPORTER_LEDGER_TOKEN` (implicit public read of a fine-grained PAT — already a §8.4 platform property); §4.4 (decide row, sweep job assignment), §8.3 and §11(b) follow the wording. (2) §11(a) listed the repository-wide `actions/runs`; with no successful scheduled sweep yet the fallback returned 1 691 runs (17 pages) and tripped the 1 000-result cap — fail-closed forever. §11(a) now names the per-workflow `actions/workflows/supporter-loop.yml/runs` listing (11 runs, one page) and forbids the repository-wide one; the cap/page-error rule is unchanged. No permission boundary is weakened: the ledger token gains no write path anywhere, the Administration token still never enters `decide`, and the exception covers one `GET`.
