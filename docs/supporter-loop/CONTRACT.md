...
on:
  ...
  workflow_dispatch:       # owner-triggered sweep (v1.11); same steps as the schedule, no inputs
permissions:
...
|---|---|---|---|
| `stars` (v1.11) | `inputs.sweep == true` only (`timeout-minutes: 5`) | `contents: write` and nothing else — the only job in the loop with `contents` above `none`, because `GET /repos/{source_repo}/stargazers` is refused to every other credential (§11(b)) | `GITHUB_TOKEN` for exactly one paginated `GET /repos/{source_repo}/stargazers`; **no** PAT, **no** Telegram secret, no checkout, no other network call. Output: the numeric stargazer ids only, plus an `ok` flag; `decide` consumes them via `needs.stars.outputs` and treats a missing/failed handoff as precondition (b) false. Pinned by child #2's interface test (one step, `--method GET`, no `secrets.` reference) |
...
| every live-vocabulary line (`invite`, `comment`, `supporters-append`, `revoke`, `cancel-invite` — including the sweep's `expired` re-arm lines and precondition (e) recovered `comment` lines) | `act` | **after** the corresponding API call returned (never before; §5.2); ledger-only lines at the point the decision is made |
...
- **Transition table** (the normative summary of this section; prose must agree with it). "Line" = the most relevant earlier line for (`dedup_key`, current `gen`, action `X`):
  ...
  | `invite` with success result, followed by `cancel-invite` `result: expired` (§11 step 2) | any tier event | `invite` delivered again (re-armed); `comment` / `supporters-append` unaffected; same generation |
...
- **Backfill** (checkpoint #4, owner-authorized, one-off): a `sweep` variant enabled by an optional boolean input `backfill` (default `false`) that child #2 adds **at checkpoint #4** — adding an optional input with a default is not a bump (§1) and the input is deliberately absent from the v1 interface so backfill is unreachable before the owner authorizes it. For every `actor_id` whose current (open) generation contains a `would-invite` and no live `invite`, and who is **still a stargazer at backfill time** (checked against the exhaustive stargazer list by numeric id), `act` executes a live `invite` (ledgered `invite`, `result: ok-backfill`, `gen` = current). Checkpoint #4 MUST show the owner this exact list. Tier-2/3 `would-comment` / `would-supporters-append` lines are never backfilled — the moment has passed; the supporter's next Issue/PR is delivered by the "rehearsal does not count" row.
...
6. **`SUPPORTERS.md` regeneration** (live only) if anything changed.
...
| Invitation preflight or `PUT` returns anything other than 200/201/204 (401 revoked token, 403 user blocked the org, 404, 422 invitation limit, 5xx) | ledger `invite`, `result: error-<status>`; the tier-2/3 **comment and `supporters-append` of that run are not sent** and are ledgered by `act` as `action: comment` / `action: supporters-append` with `result: error-<status>` (act-owned live vocabulary, so the §4.4 authorship table holds; the template would assert access that does not exist); other actors' actions continue; job ends red. The comment is delivered on the actor's next tier event once an invitation stands (the `error-*` line does not count as delivered, §7) |
...
| A red supporter-loop run exists since the last successful sweep | sweep runs steps 1, 2, 3, 5, 6, 7 (step 6 still regenerates `SUPPORTERS.md`, because precondition (e)'s recovered `comment` lines can raise an achieved tier); step 4 (and the queued API cancels) is skipped with `::error::`; the owner reconciles (re-run the red job or append the missing line by hand via PR) |
