...
Triggered by the caller's `schedule` (§4.3) with `sweep: true`. The step order below is normative (the audit must see the expiry re-arm before it judges, and the destructive step must see the audit).
...
| A red supporter-loop run exists since the last successful sweep | sweep runs steps 1, 2 (ledger-only part), 3, 5, 7; step 4 and the API cancel are skipped with `::error::`; the owner reconciles (re-run the red job or append the missing line by hand via PR) |
