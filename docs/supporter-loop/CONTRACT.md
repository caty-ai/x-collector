...
- **Generation** (per actor per source repo, shared by tiers 1–3): `gen(repo, actor_id)` = 1 + the number of **closure events**, where a closure event is one run (`run_id`) that wrote at least one line with `.repo == repo`, `.actor_id == actor_id`, `.action` in {`revoke`, `would-revoke`, `cancel-invite`, `would-cancel-invite`} **and a success result other than `expired`** — regardless of those lines' `dedup_key`. A `revoke` + `cancel-invite` pair from one sweep counts once; a failed revoke (`error-*`) closes nothing and is retried by the next sweep. A `cancel-invite` with `result: expired` (§11 step 3) is **not** a closure: it only **re-arms the `invite` action** — the actor's `invite` for the current generation is treated as not delivered from that line on, while `comment` / `supporters-append` stay delivered, so the next tier event of any tier re-issues the invitation and nothing else. The actor's next tier event of any tier after a closure opens the next generation and is rewarded again (tiers 1..N at once, §2).
...
- **Transition table** (the normative summary of this section; prose must agree with it). "Line" = the most relevant earlier line for (`dedup_key`, current `gen`, action `X`):
  ...
  | `invite` with success result, followed by `cancel-invite` `result: expired` (§11 step 3) | any tier event | `invite` delivered again (re-armed); `comment` / `supporters-append` unaffected; same generation |
