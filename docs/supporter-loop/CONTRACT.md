...
| A red supporter-loop run exists since the last successful sweep | sweep runs steps 1, 2, 3, 5, 7; step 4 (and the queued API cancels) is skipped with `::error::`; the owner reconciles (re-run the red job or append the missing line by hand via PR) |
