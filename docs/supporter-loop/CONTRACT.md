...
| `alert` | `needs: [decide, act]` with `if: ${{ !cancelled() && failure() && inputs.mode == 'live' }}` (a skipped `act` does not suppress it; `timeout-minutes: 2`) | none (`permissions: {}`) | Telegram secrets only (§9) — one message naming the failed job and the run URL; no ledger, no PATs |
