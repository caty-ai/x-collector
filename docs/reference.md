...
- **Public newspaper is opt-in** — `NEWSPAPER_PUBLIC=1` (or `true`) opens `/calendar`, its static assets, the newsletter BFF, and the og-image BFF to anonymous readers; default off. The anonymous newsletter BFF returns published editions only (drafts and empty dates are 404), og-image requests must name an edition (`?date=`, a same-origin Referer with `?date=`, or a latest-edition URL), and anonymous requests are throttled per IP (newsletter 240 / 60 s, og-image 120 / 60 s). Every other route keeps requiring an allowlisted Google account, except `/np-login`, which stays available for the shared-passphrase sign-in in both switch states
- **Recompose CLI** — `npm run recompose:script -- --date-jst=YYYY-MM-DD --dry-run [--out content.md]` re-typesets an existing edition in the deterministic script mode; drop `--dry-run` to apply. Idempotent, does not touch `PipelineRun` in dry-run, and the current day's edition is regenerated by the nightly cron anyway (`recompose:script:prod` on Railway)
