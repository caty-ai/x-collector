...
- **Discovery** — `robots.txt` allows `/a/`, `/calendar`, `/sitemap.xml`, and `/og-default.png` and points at `sitemap.xml` only when `NEWSPAPER_PUBLIC` is on and a site origin (`NEWSPAPER_SITE_URL`, fallback `NEXTAUTH_URL`) resolves; otherwise robots disallows all and the sitemap is empty. The sitemap lists article URLs for the last 7 JST days via the same server-only loader (sequential, cached), with a 5-second per-request budget checked between dates (returns a partial sitemap on timeout), and never trusts `Host`.
...
- **X follow button** — `NEWSPAPER_X_FOLLOW_HANDLE` optionally enables the official X follow button on `/a/` only (1–15 letters, digits or underscores, optional leading `@`). Unset or invalid values hide both the button and its external script.
