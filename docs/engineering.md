...
| Operating systems (develop & self-host) | WSL2 (Ubuntu on Windows) | ⚠️ Covered by the Linux path; not separately exercised. Clone under the Linux filesystem (e.g. `~/`), not `/mnt/c` — `next dev` file watching is unreliable and `npm install` is much slower over drvfs |
