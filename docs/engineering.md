...
- **Composition boundary** — Full-width `引用元：` is deliberately treated as source-less, with no article landing page; parser support is a follow-up issue. LLM-composed editions are outside the ID stability guarantee. `STEP5_COMPOSE_MODE` defaults to `script`; set `llm` explicitly to retain LLM composition.
